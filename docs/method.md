...
### 4.4 Бизнес-решение (официальный доступ)

Если для OWI критично получать `horsepower`/`drive` как **первичный источник истины**
без эвристик и локальных таблиц, то это решается не “хакерским” обходом, а
**бизнес-договорённостями**:

- **Официальный B2B доступ Encar**: договор + выдача ключей + (часто) IP whitelist.
  Это единственный вариант, при котором можно ожидать стабильный API со
  структурированными характеристиками и SLA.
- **Лицензия на каталожные данные** (например JATO Dynamics или аналог):
  Encar уже отдаёт `category.jatoVehicleId`, то есть связь с внешним каталогом у них
  существует. При наличии подписки можно получать характеристики из каталога
  независимо от того, заполняет ли Encar поле `horsePower` в своей базе.

Плюсы бизнес-пути:
- предсказуемость, минимум риска банов/капч
- покрытие “всех марок”, а не только тех, что попали в локальные таблицы

Минусы:
- деньги и сроки на юридику/подключение
- возможные ограничения по объёму и условиям использования данных
